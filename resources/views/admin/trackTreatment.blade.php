    ...
    <div class="flex-1 flex flex-col w-full">
        ...
        <main class="flex-1 p-6 " id="main-content">
            <div class="w-full">
                ...
                <form action="{{route("treatment.update", $id->id)}}" method="POST" class="grid gap-6 shadow-xl rounded-xl">
                    {{-- name section --}}
                    @csrf
                    <div class="shadow-sm rounded-xl p-8">
                        ...
                        <div class="grid grid-cols-1 max-w-xl gap-6 pt-5 ">
                            <div class="flex gap-4 justify-start items-center">
                                ...
                                <input type="text" name="treatment"  value="{{$id->treatment_offer}}" class="border-2 border-slate-400  rounded-lg shadow lg:px-10 px-4 py-3 text-left text-md w-full">
                            </div>
                        </div>  
                    </div>
                    
                    <div class=" px-4 w-full max-[520px]:max-w-[600px]  max-sm:flex-col flex gap-4 items-center justify-start pb-8">
                        <button type="submit" class="px-10 py-3 rounded-md text-center border-none text-lg text-white shadow bg-[#624E88] hover:bg-[#58457b]">Update Treatment</button>
                    </div>
