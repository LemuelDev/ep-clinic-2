    ...
    <div class="flex-1 flex flex-col w-full">
        ...
        <main class="flex-1 p-6 " id="main-content">
            <div class="w-full">
                ...
                <h1 class="text-3xl font-bold pb-4 py-2 max-lg:text-center">Edit Time Slot</h1>
               
                <form action="{{route("admin.updateTime", $id->id)}}" method="POST" class="grid gap-6 shadow-xl rounded-xl">
                    @csrf
                    @method('PATCH')
                    <div class="shadow-sm rounded-xl p-8">
                        <div class="grid grid-cols-1 max-w-xl gap-6 pt-5 ">
                            <div class="flex gap-4 justify-start items-center">
                                ...
                                <input type="time" class="form-control" id="start_time" name="start_time" value="{{  \Carbon\Carbon::parse($id->start_time)->format('H:i')  }}" required>
                            </div>
                            @error('start_time')
                            ...
                            <div class="flex gap-4 justify-start items-center">
                                ...
                                <input type="time" class="form-control" id="end_time" name="end_time" value="{{  \Carbon\Carbon::parse($id->end_time)->format('H:i') }}" required>
                            </div>
                            @error('end_time')
                                <div class="text-red-500 mt-2 text-sm">{{ $message }}</div>
                            @enderror
                    ...
                    <div class=" px-4 w-full max-[520px]:max-w-[600px]  max-sm:flex-col flex gap-4 items-center justify-start pb-8">
                        <button type="submit" class="px-10 py-3 rounded-md text-center border-none text-lg text-white shadow bg-[#624E88] hover:bg-[#58457b]">Update Time Slot</button>
                    </div>
